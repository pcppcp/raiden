...
def test_blockchain(
    ...
    manager_address = registry_proxy.add_token(
        address_decoder(token_proxy.contract.address)
    )
    ...
    assert is_same_address(token_proxy.contract.address, event_args['token_address'])
...
def test_channel_with_self(raiden_network, settle_timeout, token_addresses):
    ...
    transaction_hash = graph0.transact('newChannel', app0.raiden.address, settle_timeout)
